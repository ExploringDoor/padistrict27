// ─────────────────────────────────────────────────────────────────────
// D27 cosmetic FX — scroll reveals, count-up stats, champion confetti.
// Self-contained, dependency-free, honors prefers-reduced-motion.
//   window.D27fx.confetti()  — fire a celebration burst
// ─────────────────────────────────────────────────────────────────────
(function () {
  var reduce = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  // ── 1) Scroll reveal + count-up (via IntersectionObserver) ──
  ...
  var REVEAL = '.tcard, .hub-stat, .type-group, .ch-item, .ch-await-card, .ab-contact, .ss-card, .league-card, .bk-champion, .gn-card';
  ...
  function confetti() {
    ...
    var colors = ['#002D72', '#C9A227', '#EBCB5E', '#1E5BC6', '#ffffff', '#D62828'], parts = [];
  ...
  function maybeCelebrate() {
    if (window.__d27Celebrated || !document.querySelector('.bk-champion')) return;
    window.__d27Celebrated = true; setTimeout(confetti, 450);
  }

  window.D27fx = { confetti: confetti, arm: arm };

  function init() { arm(document); maybeCelebrate(); }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', init); else init();

  // re-arm after async content (hubs / brackets render their cards later)
  if (window.MutationObserver) {
    var tmr, mo = new MutationObserver(function () { clearTimeout(tmr); tmr = setTimeout(function () { arm(document); maybeCelebrate(); }, 200); });

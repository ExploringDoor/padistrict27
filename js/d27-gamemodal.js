...
(function (global) {
  ...
  function previewHTML(t, g, cls) {
    ...
    return `
      <div class="gm-matchup">
        <div class="gm-team ${A.tbd ? 'tbd' : ''}">${A.tbd ? '' : lg(A.name, 104)}<span class="gm-tn">${esc(A.name)}</span></div>
        <div class="gm-vs">vs</div>
        <div class="gm-team ${H.tbd ? 'tbd' : ''}">${H.tbd ? '' : lg(H.name, 104)}<span class="gm-tn">${esc(H.name)}</span></div>
      </div>
  ...
  function recapHTML(t, g, cls) {
    ...
    if (isForfeit(g)) {
      ...
      return `
        <div class="gm-byewrap">${lg(win, 104)}<div class="gm-team">${esc(win)}</div><div class="gm-meta">Won by forfeit</div></div>
        <div class="gm-sec"><h4>Recap</h4><p class="gm-recap">${esc(text)}</p></div>`;
    }
    const A = sideDisplay(t, g.away), H = sideDisplay(t, g.home);
    ...
    return `
      <div class="gm-score">
        <div class="gm-srow ${aWin ? 'win' : ''}"><span class="t">${A.tbd ? '' : lg(A.name, 52)}<span class="gm-tn">${esc(A.name)}</span></span><span class="s">${g.as}</span></div>
        <div class="gm-srow ${hWin ? 'win' : ''}"><span class="t">${H.tbd ? '' : lg(H.name, 52)}<span class="gm-tn">${esc(H.name)}</span></span><span class="s">${g.hs}</span></div>
      </div>
      <div class="gm-sec"><h4>Recap</h4><p class="gm-recap">${esc(text)}</p></div>`;
  }
